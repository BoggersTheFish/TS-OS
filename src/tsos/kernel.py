...
from .drivers import ClockedExecutionDriver, ExecutionDriver
...
class Kernel:
    ...
    def __init__(self, N: int = 12, driver: ExecutionDriver | None = None, field_enabled: bool = False):
        self.N = N
        ...
        self.voxel_ownership = np.full((N, N, N), UNOWNED, dtype=np.int32)
    ...
    def _resolve_ipc(self) -> None:
        ...
        state_hash = sha256_hex({"tick": self.runtime_tick, "owners": int(np.sum(owners))})
        for pair, count in pairs.items():
            proc_a, proc_b = pair
            self.ipc_records[pair] = IPCRecord(proc_a, proc_b, count, 0, count, self.runtime_tick, True, state_hash)
    ...
    def tick(self) -> dict[str, Any]:
        ...
        for pid in sorted(self.processes):
            ...
            allowed = self.driver.instructions_allowed_this_tick(pid, self.runtime_tick)
