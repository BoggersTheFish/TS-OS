...
# Test Pair (0, 4) and Pair (1, 3)
failed_pairs = [(0, 4), (1, 3)]
for nid_a, nid_b in failed_pairs:
    node_a = nodes[nid_a]
    node_b = nodes[nid_b]
    ...
    res_a, path_a = trace_3d_projected(midpoint, edge_dir, edge_dir)
    res_b, path_b = trace_3d_projected(midpoint, -edge_dir, edge_dir)
    print(f"\nPair ({nid_a}, {nid_b}) at dist {dist:.4f}: midpoint={midpoint}")
    print(f"  res_a={res_a} (steps={len(path_a)}), res_b={res_b} (steps={len(path_b)})")
    if res_a is None:
        print("  Path A last 5 points:")
        for p in path_a[-5:]:
            print(f"    {p}")
    if res_b is None:
        print("  Path B last 5 points:")
        for p in path_b[-5:]:
            print(f"    {p}")
