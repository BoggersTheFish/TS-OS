...
# Target saddle signature: lambda_2 < 0 and lambda_3 > 0
is_target_saddle = is_min_grad & (lambda_2 < 0.0) & (lambda_3 > 0.0) & interior_mask

indices_x, indices_y, indices_z = np.where(is_target_saddle)
print(f"Number of index-2 saddles found: {len(indices_x)}")
for ix, iy, iz in zip(indices_x, indices_y, indices_z):
    ...
    print(f"  Saddle at {pos} [val={val:.4f}, grad={g_val:.4f}]: eigenvalues = {l_1:.2f}, {l_2:.2f}, {l_3:.2f}")
