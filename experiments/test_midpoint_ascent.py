...
# Define the 12 edge midpoints of the Cube
# For cube nodes at approx (+-1.3, +-1.3, +-1.3)
d = 1.3
midpoints = [
    # 4 midpoints along X (parallel to X-axis, y,z constant)
    (0.0, d, d, (1.0, 0.0, 0.0)), (0.0, d, -d, (1.0, 0.0, 0.0)), (0.0, -d, d, (1.0, 0.0, 0.0)), (0.0, -d, -d, (1.0, 0.0, 0.0)),
    # 4 midpoints along Y
    (d, 0.0, d, (0.0, 1.0, 0.0)), (d, 0.0, -d, (0.0, 1.0, 0.0)), (-d, 0.0, d, (0.0, 1.0, 0.0)), (-d, 0.0, -d, (0.0, 1.0, 0.0)),
    # 4 midpoints along Z
    (d, d, 0.0, (0.0, 0.0, 1.0)), (d, -d, 0.0, (0.0, 0.0, 1.0)), (-d, d, 0.0, (0.0, 0.0, 1.0)), (-d, -d, 0.0, (0.0, 0.0, 1.0))
]
...
print("\nTracing from midpoints:")
for idx, (mx, my, mz, dir_vec) in enumerate(midpoints):
    dir_vec = np.array(dir_vec)
    node_a, path_a = trace_3d((mx, my, mz), dir_vec)
    node_b, path_b = trace_3d((mx, my, mz), -dir_vec)
    print(f"Midpoint {idx} at ({mx}, {my}, {mz}): node_a={node_a} (steps={len(path_a)}), node_b={node_b} (steps={len(path_b)})")
